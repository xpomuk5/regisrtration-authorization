...
class User
{
    ...
    public function create($username, $password, $email, $name) {
        ...
        if ($user_exists) {
            throw new \Exception("Пользователь существует: " . $username, 1);
        }

        $query = "insert into users (username, password, salt, email, name)
            values (:username, :password, :salt, :email, :name)";
